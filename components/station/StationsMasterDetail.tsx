        ...
        if (response.status === 200) {
          ...
          setSelection(
            (prev) =>
              prev ??
              (response.message[0]
                ? { stationUuid: response.message[0].uuid, zoneIndex: null }
                : null)
          );
  ...
  const selectedZone = useMemo(
    () =>
      selection?.zoneIndex !== null && selection?.zoneIndex !== undefined
        ? selectedStation?.zones.find((z) => z.index === selection.zoneIndex) ??
          null
        : null,
    ...
    <Stack direction="row" sx={{ minHeight: "calc(100vh - 64px)", m: 0 }}>
        ...
        >
          {stations.map((station) => (
            <Box key={station.uuid}>
              <ListItemButton
                selected={
                  selection?.stationUuid === station.uuid &&
                  selection?.zoneIndex === null
                }
                onClick={() =>
                  setSelection({ stationUuid: station.uuid, zoneIndex: null })
                }
                sx={{
                  py: 0.75,
              ...
              {station.zones.map((zone: Zone) => (
                <ListItemButton
                  ...
                  selected={
                    selection?.stationUuid === station.uuid &&
                    selection?.zoneIndex === zone.index
                  }
                  onClick={() =>
                    setSelection({
      ...
      <Box sx={{ flex: 1, p: 2, overflowY: "auto" }}>
        ...
        {selectedStation && !selectedZone && (
          <Stack gap={1.5}>
            ...
            >
              ...
              <Typography
                sx={{ fontWeight: 700, fontSize: 14, color: "primary.light" }}
              >
            ...
            >
              <PropaneTankOutlinedIcon fontSize="small" color="primary" /> Бак
            </Typography>
        ...
        {selectedStation && selectedZone && (
          <Stack gap={1}>
            <Typography
              sx={{ fontSize: 12, color: "primary.light", fontWeight: 600 }}
            >
